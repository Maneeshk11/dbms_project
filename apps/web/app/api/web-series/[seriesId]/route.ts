...
export async function GET(
  ...
  try {
    ...
    // Fetch all data in parallel using Promise.all to minimize connection usage
    const [
      seriesData,
      episodes,
      dubbingLanguagesRaw,
      subtitleLanguagesRaw,
      releasesRaw,
      feedbackRaw,
      contractsRaw,
    ] = await Promise.all([
    ...
    const dubbingLanguages = dubbingLanguagesRaw.filter(
      (lang) => lang.languageName && lang.languageCode
    );

    const subtitleLanguages = subtitleLanguagesRaw.filter(
      (lang) => lang.languageName && lang.languageCode
    );

    const releases = releasesRaw.filter((rel) => rel.countryName);

    const feedback = feedbackRaw.filter(
      (fb) => fb.viewerName && fb.viewerLastName
    );

    const contracts = contractsRaw.filter((contract) => contract.houseName);
